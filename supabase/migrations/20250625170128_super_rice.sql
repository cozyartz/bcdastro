/*
  # Battle Creek Drone Media Platform Database Schema

  1. New Tables
    - `users` - Extended user profiles linked to Supabase auth
    - `media_assets` - Individual photos and videos with pricing tiers
    - `media_packages` - Property collections (like golf courses) with package pricing
    - `purchases` - Transaction records with license types
    - `download_logs` - Detailed download tracking for analytics

  2. Security
    - Enable RLS on all tables
    - Policies for user data access and media visibility
    - Anonymous access for approved, non-exclusive content

  3. Features
    - Multi-tier pricing (individual, package, exclusive)
    - Property-based media organization
    - Search optimization with text search
    - Revenue and download tracking
    - Automated user statistics updates
*/
...
-- Function for text search on media assets
CREATE OR REPLACE FUNCTION search_media_assets(search_query text)
RETURNS TABLE (
  id uuid,
  title text,
  description text,
...
BEGIN
  ...
  SELECT 
    ...
    ma.tags,
    ts_rank(
      to_tsvector('english', ma.title || ' ' || ma.description || ' ' || array_to_string(ma.tags, ' ')),
      plainto_tsquery('english', search_query)
    ) as rank
  FROM media_assets ma
  WHERE 
    ma.status = 'approved' 
    AND NOT ma.is_exclusive
    AND (
      to_tsvector('english', ma.title || ' ' || ma.description || ' ' || array_to_string(ma.tags, ' '))
      @@ plainto_tsquery('english', search_query)
    )
